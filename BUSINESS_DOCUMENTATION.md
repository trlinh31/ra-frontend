...
- Quản lý lead / yêu cầu chuyến đi từ khách hàng (TripRequest) — theo dõi toàn bộ phễu từ tiếp nhận đến chốt hợp đồng
...
Màn hình tổng quan sau khi đăng nhập, hiển thị:

- **KPI Cards:** Số báo giá đang mở, số tour đang vận hành, doanh thu đã thu, công nợ vendor chưa thanh toán
- **Biểu đồ trạng thái:** Donut chart theo trạng thái Quotation và ConfirmedTour; progress bar tổng hợp tài chính
- **Công việc cần xử lý gấp:** Thanh toán quá hạn, tour chờ duyệt, báo giá chờ phản hồi
- **Tour sắp khởi hành:** Danh sách các ConfirmedTour sắp diễn ra
- **Vendor sắp đến hạn thanh toán:** Cảnh báo phiếu chi gần hạn
- **Nút tác vụ nhanh:** Tạo TripRequest, Tạo Báo giá, Tạo Confirm Tour
...
| STT | Chức năng        | Mô tả                                                                                       |
| --- | ---------------- | ------------------------------------------------------------------------------------------- |
| 1   | **Trip Request** | Tiếp nhận và quản lý yêu cầu chuyến đi từ khách hàng; theo dõi phễu lead đến khi chuyển đổi |
| 2   | **Báo giá**      | Tạo và gửi báo giá tour cho khách hàng; quản lý vòng đàm phán đến khi khách chấp thuận      |
| 3   | **Confirm Tour** | Tạo tour thực tế từ báo giá đã được chấp thuận, tùy chỉnh theo yêu cầu đoàn khách cụ thể    |

**Phễu bán hàng Sales (Sales Funnel):**

```
Trip Request ──► Quotation ──► Confirmed Tour ──► In Operation ──► Completed
    (Lead)        (Báo giá)      (Chốt tour)       (Vận hành)      (Hoàn thành)
```
...
Mỗi user có các thông tin: `id`, `name`, `username`, `email`, `role`, `avatar`, `isActive`. Phân quyền được gán thông qua trường `role` (xem mục 4).
...
### 3.7 Quy trình TripRequest – Tiếp nhận yêu cầu chuyến đi ✨ _Mới_

> **Actor:** Seller, Sale Manager  
> **Tiền điều kiện:** Seller nhận được yêu cầu từ khách hàng qua bất kỳ kênh nào  
> **Hậu điều kiện:** TripRequest được lưu vào hệ thống; Seller có thể tạo Quotation từ đây

**Tổng quan vị trí trong phễu Sales:**

```
[Khách liên hệ] ──► TripRequest ──► Quotation ──► ConfirmedTour
   (ngoài HT)        (lead)          (báo giá)      (chốt tour)
        │
        │  qua các kênh:
        │  email / phone / Zalo / giới thiệu / walk-in
        ▼
   Seller nhập thủ công vào hệ thống
```

**Luồng xử lý chi tiết:**

```
Bước 1: Seller hoặc Sale Manager nhấn [+ Tạo Trip Request]
Bước 2: Nhập thông tin khách hàng:
        - Tên khách / tên đoàn (bắt buộc)
        - Email, số điện thoại liên hệ
        - Nguồn lead (email / phone / Zalo / giới thiệu / walk-in / khác)
Bước 3: Nhập nhu cầu chuyến đi:
        - Điểm đến mong muốn
        - Ngày đi dự kiến
        - Số ngày dự kiến
        - Số lượng người (người lớn / trẻ em)
        - Mức dịch vụ: budget / standard / luxury
        - Sở thích, yêu cầu đặc biệt (ăn chay, phòng riêng, xe lăn...)
        - Ngân sách tham khảo (nếu khách cung cấp)
Bước 4: Chọn tour mẫu gợi ý (không bắt buộc):
        - Seller có thể chọn 1 tour mẫu phù hợp làm cơ sở
        → Khi tạo Quotation từ TripRequest này, tour mẫu được tự động điền
Bước 5: Assign Seller phụ trách:
        - Sale Manager phân công Seller cụ thể, HOẶC
        - Nếu không có Sale Manager → Seller tự nhận (mặc định người tạo)
Bước 6: Lưu TripRequest → trạng thái "new" hoặc "assigned"
Bước 7: Seller xử lý lead:
        - Liên hệ lại khách, trao đổi chi tiết
        - Ghi chú nội bộ cập nhật tiến độ
        - Nhấn [+ Tạo Báo giá] → chuyển sang quy trình 3.8
```

**Vòng đời trạng thái TripRequest:**

```
new ──► assigned ──► in_progress ──► converted
                          │           (có ConfirmedTour)
                          ├──► lost
                          │    (khách từ chối / không liên lạc được)
                          └──► on_hold
                               (khách tạm hoãn, hẹn liên hệ lại)
```
...
| ------------- | ----------------------------------------- | --------------------------------- |
| `new`         | Vừa tạo, chưa assign Seller               | Sale Manager assign Seller        |
| `assigned`    | Đã có Seller phụ trách, chưa xử lý        | Seller liên hệ khách              |
| `in_progress` | Đang trong quá trình tư vấn / tạo báo giá | Tạo Quotation                     |
| `converted`   | Đã chốt — có ConfirmedTour liên kết       | —                                 |
| `lost`        | Mất lead — khách không chốt               | Ghi lý do mất                     |
| `on_hold`     | Tạm hoãn — khách hẹn liên hệ lại sau      | Hệ thống nhắc Seller theo hạn hẹn |

**Liên kết với các module khác:**

```
TripRequest.quotationIds[] ──► (nhiều) Quotation   [liên kết một chiều từ TripRequest]
TripRequest.confirmedTourId ──► (1) ConfirmedTour  [khi converted]
TripRequest.suggestedTourId ──► (1) Tour mẫu gợi ý [optional]
```

> **Lưu ý:** TripRequest **không bắt buộc** — Seller vẫn có thể tạo Quotation hoặc ConfirmedTour
> trực tiếp mà không cần TripRequest. TripRequest chỉ là công cụ quản lý lead, không phải
> điều kiện tiên quyết bắt buộc trong luồng.

---

### 3.8 Quy trình Báo giá (Phòng Sales) ✨ _Mới_

> **Actor:** Seller  
> **Tiền điều kiện (Pre-condition):** Có TripRequest ở trạng thái `in_progress` (khuyến nghị) **hoặc** Seller tạo trực tiếp không qua TripRequest  
> **Hậu điều kiện (Post-condition):** Khách hàng chấp thuận → TripRequest chuyển `converted` → tạo Confirm Tour (3.9)
...
### 3.9 Quy trình Confirm Tour (Phòng Sales)

> **Actor:** Seller, Sale Manager  
> **Tiền điều kiện:** Có Quotation ở trạng thái `approved` **hoặc** Seller có quyền confirm trực tiếp không qua báo giá  
> **Hậu điều kiện:** ConfirmedTour ở trạng thái `confirmed` → TripRequest gốc (nếu có) chuyển `converted` → sẵn sàng để Assign vận hành
...
### 3.10 Quy trình Assign Tour cho Vận hành
...
### 3.11 Quy trình Follow Tour & Sửa Detail (Phòng Vận hành)
...
### 3.12 Quy trình Follow Thanh toán (Phòng Kế toán)

**3.12A – Thu tiền từ khách hàng:**
...
### 3.13 Quy trình Hủy Tour & Hoàn tiền
...
### 3.14 Quy trình quản lý Dịch vụ thêm (Add-on) trong Master Data
...
| `createdBy`         | string               | Người tạo phiếu thu   |
...
| `tourTemplateId`   | string?                 | ID tour mẫu làm cơ sở (nếu có)                     |
| `tourTemplateName` | string?                 | Tên tour mẫu làm cơ sở (nếu có)                    |
...
| `confirmedTourId`  | string?                 | ID ConfirmedTour được tạo từ báo giá này (nếu có)  |
...
| Giá trị       | Ý nghĩa                                                  |
| ------------- | -------------------------------------------------------- |
| `new`         | Vừa tạo, chưa assign Seller                              |
| `assigned`    | Đã có Seller phụ trách, chưa xử lý                       |
| `in_progress` | Đang tư vấn / đã tạo ít nhất 1 Quotation                 |
| `converted`   | Đã chốt — có ConfirmedTour liên kết                      |
| `lost`        | Mất lead — khách không chốt, ghi rõ lý do                |
| `on_hold`     | Tạm hoãn — khách hẹn liên hệ lại theo ngày `onHoldUntil` |
...
_Tài liệu này được tổng hợp và cập nhật từ phân tích source code frontend + thiết kế nghiệp vụ. Mọi thay đổi nghiệp vụ cần được cập nhật đồng bộ tại đây._
