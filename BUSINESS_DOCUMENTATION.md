...
```
TripRequest (1) ──► (nhiều) Quotation
TripRequest (1) ──► (1) ConfirmedTour  [khi converted]
TripRequest (n) ──► (1) Tour mẫu gợi ý [optional]
```
...
| `tripRequestId`    | string?                 | Tham chiếu TripRequest gốc (nếu có)                |
...
| `customerEmail`           | string? | Email liên hệ                                                             |
| `customerPhone`           | string? | Số điện thoại liên hệ                                                     |
| `source`                  | enum    | Nguồn lead: `email` / `phone` / `zalo` / `referral` / `walk_in` / `other` |
| `destination`             | string? | Điểm đến mong muốn                                                        |
| `estimatedDeparture`      | string? | Ngày đi dự kiến                                                           |
| `duration`                | number? | Số ngày dự kiến                                                           |
| `numberOfPeople`          | number? | Số người (tổng)                                                           |
| `numberOfAdults`          | number? | Số người lớn                                                              |
| `numberOfChildren`        | number? | Số trẻ em                                                                 |
| `serviceLevel`            | enum?   | Mức dịch vụ: `budget` / `standard` / `luxury`                             |
| `budget`                  | string? | Ngân sách tham khảo (mô tả tự do, VD: "khoảng 2000 USD/người")            |
| `interests`               | string? | Sở thích, yêu cầu đặc biệt (ăn chay, phòng riêng, xe lăn...)              |
| `suggestedTourTemplateId` | string? | Tour mẫu gợi ý (tùy chọn)                                                 |
| `assignedTo`              | string? | Seller phụ trách                                                          |
| `status`                  | enum    | Trạng thái (xem bảng bên dưới)                                            |
| `lostReason`              | string? | Lý do mất lead (chỉ khi `lost`)                                           |
| `onHoldUntil`             | string? | Ngày hẹn liên hệ lại (chỉ khi `on_hold`)                                  |
| `convertedTourId`         | string? | ConfirmedTour đã tạo (chỉ khi `converted`)                                |
| `note`                    | string? | Ghi chú nội bộ                                                            |
...
| 14  | **TripRequest → Quotation**     | Khi tạo Quotation từ TripRequest, hệ thống tự điền thông tin khách và tour mẫu gợi ý từ TripRequest.                                       |
