import EntranceFeeDayGroupsForm from "@/modules/masterData/entranceFee/components/EntranceFeeDayGroupsForm";
...
interface EntranceFeePricingPeriodFormProps {
  periodIndex: number;
}

export default function EntranceFeePricingPeriodForm({ periodIndex }: EntranceFeePricingPeriodFormProps) {
        ...
        <div className='space-y-4'>
          {dateRangeFields.map((field, rangeIdx) => (
            <div key={field.id} className='space-y-4 p-4 border border-gray-300 border-dashed rounded-lg'>
              ...
              <EntranceFeeDayGroupsForm periodIndex={periodIndex} rangeIndex={rangeIdx} />
            </div>
          ))}
        </div>
