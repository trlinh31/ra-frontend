import GroupTourDayGroupsForm from "@/modules/masterData/groupTour/components/GroupTourDayGroupsForm";
...
interface GroupTourPricingPeriodFormProps {
  periodIndex: number;
}

export default function GroupTourPricingPeriodForm({ periodIndex }: GroupTourPricingPeriodFormProps) {
  const { control, formState } = useFormContext<GroupTourFormValues>();
        ...
        <div className='space-y-4'>
          {dateRangeFields.map((field, rangeIdx) => (
            <div key={field.id} className='space-y-4 p-4 border border-gray-300 border-dashed rounded-lg'>
              ...
              <GroupTourDayGroupsForm periodIndex={periodIndex} rangeIndex={rangeIdx} />
            </div>
          ))}
        </div>
