import RestaurantPricingPeriodForm from "@/modules/masterData/restaurant/components/RestaurantPricingPeriodForm";
...
export default function RestaurantPricingForm() {
  ...
  const handleAddPeriod = () => {
    append({
      currency: "VND",
      dateRanges: [{ from: "", to: "", dayGroups: [{ label: "", days: [], price: undefined as unknown as number, comboPackageIndex: "" }] }],
    });
  };
      ...
      {fields.map((field, periodIndex) => {
        ...
        return (
          <Section key={field.id} type='dashed' borderColor='red' className='relative'>
            ...
            <RestaurantPricingPeriodForm periodIndex={periodIndex} />
          </Section>
        );
      })}
