import RestaurantDayGroupsForm from "@/modules/masterData/restaurant/components/RestaurantDayGroupsForm";
...
interface RestaurantPricingPeriodFormProps {
  periodIndex: number;
}

export default function RestaurantPricingPeriodForm({ periodIndex }: RestaurantPricingPeriodFormProps) {
        ...
        <div className='space-y-4'>
          {dateRangeFields.map((field, rangeIdx) => (
            <div key={field.id} className='space-y-4 p-4 border border-gray-300 border-dashed rounded-lg'>
              ...
              <RestaurantDayGroupsForm periodIndex={periodIndex} rangeIndex={rangeIdx} />
            </div>
          ))}
        </div>
        ...
        <ActionButton
          ...
          onClick={() =>
            appendDateRange({
              from: "",
              to: "",
              dayGroups: [{ label: "", days: [], price: undefined as unknown as number, comboPackageIndex: "" }],
            })
          }
        />
