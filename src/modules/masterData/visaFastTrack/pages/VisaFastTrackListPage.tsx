...
export default function VisaFastTrackListPage() {
  ...
  const [filters, setFilters] = useState<VisaFastTrackFilters>({ serviceName: "", group: "" });
  ...
  const handleDelete = async (item: VisaService) => {
    const ok = await confirm({
      description: "Bạn có chắc chắn muốn xóa dịch vụ này không? Hành động này không thể hoàn tác.",
    });
    if (!ok) return;
  ...
  const columns: ColumnDef<VisaService>[] = [
    ...
    { header: "Giá (VNĐ)", accessorKey: "price", cell: ({ row }) => formatNumberVN(row.original.price) },
  ...
  return (
    <div className='space-y-4'>
      ...
      <VisaFastTrackFilterBar onFilter={setFilters} />
