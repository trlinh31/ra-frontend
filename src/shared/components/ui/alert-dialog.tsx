  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        data-size={size}
        className={cn(
          "group/alert-dialog-content top-1/2 left-1/2 z-50 fixed gap-4 grid bg-background p-4 rounded-xl outline-none ring-1 ring-foreground/10 w-full data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm -translate-x-1/2 -translate-y-1/2 data-closed:animate-out data-open:animate-in duration-100 data-open:fade-in-0 data-open:zoom-in-95 data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
        {...props}
      />
...
function AlertDialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex sm:flex-row flex-col-reverse sm:justify-end gap-2 group-data-[size=sm]/alert-dialog-content:grid group-data-[size=sm]/alert-dialog-content:grid-cols-2 bg-muted/50 -mx-4 -mb-4 p-4 border-t rounded-b-xl",
        className
      )}
      {...props}
    />
...
function AlertDialogTitle({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Title>) {
  return (
    <AlertDialogPrimitive.Title
      ...
      className={cn(
        "sm:group-data-[size=default]/alert-dialog-content:group-has-data-[slot=alert-dialog-media]/alert-dialog-content:col-start-2 font-semibold text-base",
        className
      )}
      {...props}
    />
